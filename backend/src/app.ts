...
postgresConnectionSequelize
  ...
  app.get('/send-notifications', async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const messages: { to: string; sound: string; body: string; data: any , title: string }[] = [];
      ...
      for (let i = 0; i < tickets.length; i++) {
        ...
        if (ticket.status === 'ok') {
          await Notification.update({ pushed: 'Yes' }, { where: { id: notificationId } });
